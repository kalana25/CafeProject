...
{
    public partial class ManageBevarage : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (mode == "New")
            {
                int result = bevarage.insert(txtcode.Text.Trim(), txtname.Text.Trim(), decimal.Parse(txtprice.Text.Trim()), txtdesc.Text.Trim());
                if(result>0)
                {
                    gvBevarage.DataSource = bevarage.Retrieve();
                    gvBevarage.Refresh();
                    ClearTextbox();
            ...
            {
                int result = bevarage.Edit(int.Parse(txtid.Text), txtcode.Text, txtname.Text, decimal.Parse(txtprice.Text), txtdesc.Text);
                if (result > 0)
                {
                    gvBevarage.DataSource = bevarage.Retrieve();
                    gvBevarage.Refresh();
        ...
        private void ManageBevarage_Load(object sender, EventArgs e)
        {
            DataTable result = bevarage.Retrieve();
            gvBevarage.DataSource = result;
            gvBevarage.Refresh();
            btnSave.Enabled = false;
            btnDelete.Enabled = false;
            txtid.Enabled = false;
        ...
        private void gvBevarage_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                int rowsAffected = bevarage.Delete(int.Parse(txtid.Text));
                if(rowsAffected>0)
                {
                    gvBevarage.DataSource = bevarage.Retrieve();
                    gvBevarage.Refresh();
